...
def add_box( points, x, y, z, width, height, depth ):
    ...
    # right
    add_polygon( points,x1, y, z,x1, y1, z1,x1, y, z )
...
# use the latitude and use the thing with the same index to construct the triangle
# 2 cases: poles and sides
# create points in generate -> add
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            if not longt == 0 or not longt == step - 2:
                add_polygon(edges, points[topind][0],
                points[topind][1],
                points[topind][2],
                points[index+1][0],
                points[index+1][1],
...
def add_torus( edges, cx, cy, cz, r0, r1, step ):
    ...
    for lat in range(lat_start, lat_stop):
        for longt in range(longt_start, longt_stop):
            ...
            nextind = index+1
            topind = (lat+1) * step + longt
            botind = (lat-1) * step + longt
            next = lat_stop*step

            if lat == lat_start:
                botind = (lat_stop-1) * step + longt
            if lat == lat_stop - 1:
                topind=longt
            if (nextind == next):
                nextind = 0
